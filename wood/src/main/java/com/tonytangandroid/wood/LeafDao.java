...
abstract class LeafDao {
  ...
  public DataSource.Factory<Integer, Leaf> getAllTransactionsWith(
      String key, @IntRange(from = 2, to = 7) int priority) {
  ...
  @Query(
      "SELECT id,createAt,length,priority,body FROM Leaf WHERE (tag LIKE :endWildCard OR body LIKE :doubleWildCard) AND priority >= :priority ORDER BY id DESC")
  abstract DataSource.Factory<Integer, Leaf> getAllTransactionsIncludeRequestResponse(
      String endWildCard, String doubleWildCard, int priority);
}
